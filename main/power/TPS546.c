...
{
    return i2c_bitaxe_register_read(tps546_dev_handle[i2c_addr], command, data, 1);
}
...
{
    return i2c_bitaxe_register_write_byte(tps546_dev_handle[i2c_addr], command, data);
}
...
{
    return i2c_bitaxe_register_write_addr(tps546_dev_handle[i2c_addr], command);
}
...
{
    uint8_t data[2];
    if (i2c_bitaxe_register_read(tps546_dev_handle[i2c_addr], command, data, 2) != ESP_OK) {
        return ESP_FAIL;
    } else {
...
{
    return i2c_bitaxe_register_write_word(tps546_dev_handle[i2c_addr], command, data);
}
...
static esp_err_t smb_read_block(uint8_t command, uint8_t *data, uint8_t len, int i2c_addr)
{
    ...
    if (i2c_bitaxe_register_read(tps546_dev_handle[i2c_addr], command, buf, len+1) != ESP_OK) {
        free(buf);
        return ESP_FAIL;
    }
...
esp_err_t TPS546_init(TPS546_CONFIG config, int i2c_addr)
{
    ...
    uint16_t status;
    TPS546_check_status(&status, i2c_addr);

    if (status != 0) {
        ESP_LOGE(TAG, "Status error: %04x", status);
        TPS546_parse_status(status, i2c_addr);
    }
...
esp_err_t TPS546_clear_faults(int i2c_addr) {
    ...
    if (smb_write_addr(PMBUS_CLEAR_FAULTS, i2c_addr) != ESP_OK) {
...
void TPS546_write_entire_config(int i2c_addr)
{
    ...
    smb_write_block(PMBUS_MFR_ID, MFR_MODEL, 3, i2c_addr);
    ESP_LOGI(TAG, "Setting MFR REVISION");
    smb_write_block(PMBUS_MFR_ID, MFR_REVISION, 3, i2c_addr);
...
int TPS546_get_temperature(int i2c_addr)
{
    ...
    ESP_LOGI(TAG, "VREG_%i Temp: %i", i2c_addr, TPS546_get_temperature(i2c_addr));
    return temp;
}
...
esp_err_t TPS546_check_status(uint16_t *status, int i2c_addr) {

    if (smb_read_word(PMBUS_STATUS_WORD, status, i2c_addr) != ESP_OK) {
